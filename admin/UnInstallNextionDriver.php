...
if ($_SERVER["PHP_SELF"] == "/admin/UnInstallNextionDriver.php") {
   ...
   if (!isset($_GET['ajax'])) {
    if (!file_exists('/var/log/pi-star')) {
     ...
     system('sudo touch /var/log/pi-star/UnInstallNextionDriver.log > /dev/null 2>&1 &');
     system('sudo echo "" > /var/log/pi-star/UnInstallNextionDriver.log > /dev/null 2>&1 &'); 
     system('sudo curl https://www.bi7jta.cn/files/AndyTaylorTweet/updateScripts/Remove_NextionDriver.sh | sudo sh > /dev/null 2>&1 &');
  }
  ...
  if (!isset($_GET['ajax'])) {
    ...
    if (file_exists('/var/log/pi-star/UnInstallNextionDriver.log')) {
      $_SESSION['update_offset'] = filesize('/var/log/pi-star/UnInstallNextionDriver.log');
    } else {
  ...
  if (isset($_GET['ajax'])) {
    ...
    $handle = fopen('/var/log/pi-star/UnInstallNextionDriver.log', 'rb');
...
?>
  ...
  <head>
    ...
    <meta name="Description" content="Pi-Star UnInstallNextionDriver" />
...
<link rel="shortcut icon" href="/images/favicon.ico" type="image/x-icon" />
    ...
    $(function() {
      $.repeat(1000, function() {
        $.get('/admin/UnInstallNextionDriver.php?ajax', function(data) {
  ...
  <body>
	  ...
	  <div class="header">
          ...
          <a class="menuexpert" href="/admin/InstallNextionDriver.php">安装Nextion驱动</a>  

...
if (!empty($_POST["submit_mode"]) && empty($_POST["mode_sel"])) { //handler for nothing selected
	...
	<tr>
          ...
          <td>
            ...
            <label for="mode-sel-1">YSF</label>'.(($YSF=='0' && file_exists($YSF_paused)?" <span class='paused-mode-span'>(Paused)</span>":"")).'
            ...
            <label for="mode-sel-2">D-Star</label> '.(($DSTAR=='0' && file_exists($DSTAR_paused)?" <span class='paused-mode-span'>(Paused)</span>":"")).'
            | <input name="mode_sel" '.(($P25=='0' && !file_exists($P25_paused)?'disabled="disabled"':"")).' access="false" id="mode-sel-3"  value="P25" type="radio">
            <label for="mode-sel-3">P25</label> '.(($P25=='0' && file_exists($P25_paused)?" <span class='paused-mode-span'>(Paused)</span>":"")).'
            | <input name="mode_sel" '.(($NXDN=='0' && !file_exists($NXDN_paused)?'disabled="disabled"':"")).' access="false" id="mode-sel-4"  value="NXDN" type="radio">
            <label for="mode-sel-4">NXDN</label> '.(($NXDN=='0' && file_exists($NXDN_paused)?" <span class='paused-mode-span'>(Paused)</span>":"")).'
            ...
            <label for="mode-sel-5">POCSAG</label> '.(($POCSAG=='0' && file_exists($POCSAG_paused)?" <span class='paused-mode-span'>(Paused)</span>":"")).' ]
